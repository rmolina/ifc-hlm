...
@dataclass
class Model1NodeParameters(NodeParameters):
...
@dataclass
class Model1States(ModelStates):
    """Model1 states."""
    ...
    q_pl: float = 0.0  # ponded to link
    q_pt: float = 0.0  # ponded to topsoil
    q_ts: float = 0.0  # topsoil to subsurface
    q_sl: float = 0.0  # subsurface to link


@dataclass
class Model1Parameters(ModelParameters):  # pylint:disable=too-many-instance-attributes
    """Model1 global parameters."""
...
def create_model1_bmi_fields():
    return BmiFields(
        component_name="HLM#1",
        input_var_names=(
        ...
        output_var_names=(
            ...
            "water_flux__ponded_to_link",
            "water_flux__ponded_to_topsoil",
            "water_flux__topsoil_to_subsurface",
            "water_flux__subsurface_to_link",
        ),
        output_var_units=(
            "m3 s-1",
            "m",
...
@dataclass
class Model1Forcings(ModelForcings):
    """Model1 forcings."""
...
@dataclass
class Model1(HlmBmi[Model1Parameters, Model1Forcings, Model1States]):
    """Model1 class."""

    parameters: Model1Parameters = field(default_factory=Model1Parameters)
    forcings_data: Model1Forcings = field(default_factory=Model1Forcings)
    initial_values: Model1States = field(default_factory=Model1States)

    bmi_fields: BmiFields = field(default_factory=create_model1_bmi_fields)
    ...
    def equations(
        ...
        node_param: Model1NodeParameters = self.network.nodes[node_id]["parameters"]
        states = Model1States.from_array(y)
        ...
        discharge = self.get_discharge(
            ...
            node_param,
            storage_fluxes,
            children_solutions if self.parameters.routing else [],
        )
        ...
        return np.array(
            [
                ...
                topsoil,
                subsurface,
                -states.q_pl + storage_fluxes.q_pl,
                -states.q_pt + storage_fluxes.q_pt,
                -states.q_ts + storage_fluxes.q_ts,
                -states.q_sl + storage_fluxes.q_sl,
            ]
        )
    ...
    def get_storage_fluxes(
        self,
        states: Model1States,
        node_param: Model1NodeParameters,
    ) -> StorageFluxes:
    ...
    def get_discharge(
        ...
        for child_states, child_flow_partition in children_solutions:
            child_states_arr = Model1States.from_array(child_states)
    ...
    def _calculate_additional_network_parameters(self) -> None:
        for node_id in self.network.nodes:
            ...
            self.network.nodes[node_id]["parameters"] = Model1NodeParameters(
